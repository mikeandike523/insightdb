...
export function getConn(): Driver {
  if(_conn){
      ...
      if(!process.env.NEO4J_URI){
          throw new Error('NEO4J_URI is not set. Note: getConn() is server-side only.')
      }else{
          _conn = neo4j.driver(process.env.NEO4J_URI, neo4j.auth.basic(process.env.NEO4J_USERNAME??"", process.env.NEO4J_PASSWORD??""))
          return _conn as Driver
      }
  }
}

export async function read(cypher: string, params = {}): Promise<SerializableObject> {
  // 1. Open a session
...
export async function write(cypher: string, params = {}): Promise<SerializableObject> {
  // 1. Open a session
  const session = getConn().session()
...
export async function clientRead(cypher: string, params = {}): Promise<SerializableObject> {
  let data = (await axios.post("/api/testing/neo4j-console",{
      queryType:"read",
      query: cypher,
      params: params
  })).data
...
export async function clientWrite(cypher: string, params = {}): Promise<SerializableObject> {
  let data = (await axios.post("/api/testing/neo4j-console",{
      queryType:"write",
      query: cypher,
      params: params
